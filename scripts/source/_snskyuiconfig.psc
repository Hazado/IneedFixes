...
Function ApplySettings()
	...
	Else
		...
		_SNQuest.ForceFollowerConsume = True
	EndIf
	;If Disease														what to do with disease?
...
Event OnPageReset(String Page)
	...
	If Page == "$Basics"
		...
		If Status == 1
			...
			If SetTimescale
				TimescaleValueOption = AddSliderOption("", TimescaleValue as float)
			Else
				AddEmptyOption()
			EndIf
	...
	ElseIf Page == "$Difficulty"
		If Status == 1
			...
			;DiseaseOption = AddToggleOption("$Dangerous Diseases", Disease)												disease
	...
	ElseIf Page == "$Guide"
		...
		;InfoCureDangDiseaseOption = AddTextOption("$Curing Dangerous Diseases", None)													;disease
		SetCursorPosition(1)
	EndIf
EndEvent
